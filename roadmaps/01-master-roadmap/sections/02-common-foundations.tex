...
\newpage
% ══════════════════════════════════════════════════════════════════════════════
\roadmapSubSection{0.4}{Advanced Diagnostics --- Oscilloscope, Thermal, Logic Analyzer}{Ongoing from Week 8}

{\small\itshape\color{arligray} Start building these skills in parallel from Week 8. They multiply diagnostic speed and accuracy by 10x.}

\roadmapHead{Oscilloscope --- Buy at Phase 4}
\begin{toolbadges}
...
\roadmapHead{Oscilloscope Fundamentals}
\checkitem{Bandwidth: signal frequency must be < scope bandwidth / 3 for accurate amplitude. 50MHz scope: accurate to ~17MHz.}{}
\checkitem{10:1 probe: 10x attenuation, 10x higher input impedance (10M$\Omega$). Use for all voltage measurements.}{}
\checkitem{Probe compensation: adjust probe trimmer cap until square wave edges are sharp before any measurement.}{}
\checkitem{DC coupling: see rail voltage + ripple. AC coupling: zoom into ripple only. Use both together.}{}
\checkitem{Acceptable ripple: 1--5\% of DC voltage. 1.8V rail: ripple < 90mV. High ripple = failing filter cap.}{}
\checkitem{Power sequencing trace: 4-channel scope. CH1=VSYS, CH2=PP1V8, CH3=PP3V3, CH4=CPU\_PWR\_OK. One capture, all rising edges.}{}
...
\checkitem{Phase node measurement: probe junction of high-side FET drain and low-side FET source (before inductor). Should be a square wave.}{}

\roadmapHead{Thermal Imaging --- Buy at Phase 4}
\begin{toolbadges}
...
\roadmapHead{Thermal Camera Usage}
\checkitem{Connect current-limited supply (0.5--1A) to shorted rail. Power briefly (3--5 seconds). Immediately photograph with thermal cam.}{Shorted component heats first. Capacitor short = pinpoint heat. IC internal short = whole package heats uniformly.}
\checkitem{Emissivity: shiny metal reads low (0.05--0.3). Black IC packages: 0.95. Apply kapton tape for accurate temperature reading.}{}
\checkitem{After BGA reflow: power on, apply load, thermal image. IC should heat uniformly. Cold corner = open joint.}{}
\checkitem{Unexpected cold component during operation: not receiving power. Unexpected hot spot: current path that shouldn't exist.}{}

\roadmapHead{Logic Analyzer --- Buy at Phase 4}
\begin{toolbadges}
\toolbadge{Saleae Logic 8 (8-channel, 100MHz)}
\toolbadge{DreamSourceLab DSLogic Plus (16-channel, budget)}
\end{toolbadges}

\roadmapHead{Logic Analyzer Usage}
\checkitem{I2C capture: CH0=SDA, CH1=SCL. Add decoder. Read: ``0x36 W: ACK, 0x03, ACK'' = write to device 0x36, register 0x03.}{}
\checkitem{SPI capture: CH0=CLK, CH1=MOSI, CH2=MISO, CH3=CS. CS never toggling = SPI master not initiating.}{}
\checkitem{UART capture: CH0 on TX test point. Set baud rate. Wrong baud = garbled output. Measure bit time on scope first.}{}
\checkitem{BIOS SPI traffic: capture during boot. No SPI activity = CPU not executing --- dead BIOS chip or dead CPU.}{}
\checkitem{Boot log search: grep for ``error'', ``fail'', ``panic'', ``timeout'', ``reset''. These are fault indicators.}{}

\roadmapHead{Power Sequencing --- Advanced}
\checkitem{PGOOD (Power Good): open-drain output asserts high once output rail is in regulation. Next rail waits for PGOOD.}{}
\checkitem{Trace sequence fault: last rail to rise before stall = fault domain. Check EN pin of stalled rail.}{}
\checkitem{Find failed PGOOD: trace back to which rail's PGOOD is stuck low. That rail is not reaching regulation.}{Common cause: shorted output cap pulls rail below threshold $\to$ PGOOD never asserts $\to$ entire sequence stalls.}
\checkitem{Read PMIC fault register via I2C after unexpected shutdown: contains overcurrent, thermal, watchdog events.}{}

\roadmapHead{Ultrasonic Cleaning --- Liquid Damage Protocol}
\begin{toolbadges}
\toolbadge{GT Sonic P3 / P6 (600ml, 40kHz)}
\toolbadge{Cleaning solution: diluted IPA or VIGON EFM}
\end{toolbadges}

\checkitem{Step 1: do not power on. Pull battery immediately. Liquid + power = corrosion in seconds.}{}
...
\checkitem{Step 4: ultrasonic clean at 40kHz with diluted IPA or VIGON EFM. 40°C, 5--10 minutes.}{}
\checkitem{Step 5: rinse with clean IPA after ultrasonic to remove cleaning solution residue.}{}
\checkitem{Step 6: dry with hot air at 60°C for 5 minutes. Then 30 minutes in silica drying cabinet.}{}
\checkitem{Step 7: visual inspection under microscope. Identify corroded vias, damaged pads, lifted components.}{}
